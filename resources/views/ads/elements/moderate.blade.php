        ...
        <div class="ui fluid card">
            ...
            <div class="extra content">
                <div class="ui bulleted list">
                    ...
                    @if (isset($ad->skills))
                        <div class="item">{{ $ad->skills }}</div>@endif
                    @if (isset($ad->languages))
                        <div class="item">{{ $ad->languages }}</div>@endif
                    @if (isset($ad->contact_phone))
                        <div class="item">{{ $ad->contact_phone }}</div>@endif
                </div>
            </div>
            <div class="ui bottom attached three compact buttons">
                <a class="ui green button moderation"
                   href="{{ action('ModerationController@accept', $ad->url) }}">{{ trans('general.buttons.accept') }}</a>
                <a class="ui orange button"
                   href="{{ action('AdController@edit', $ad->url) }}">{{ trans('general.buttons.edit') }}</a>
                <a class="ui red button moderation"
                   href="{{ action('ModerationController@refuse', $ad->url) }}">{{ trans('general.buttons.refuse') }}</a>
            </div>
        </div>
    </div>
@empty
